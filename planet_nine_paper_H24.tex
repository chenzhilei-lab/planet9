...
Three candidate sources are examined to identify where the divergence originates:
...
\paragraph{Survey selection bias.}
As reported above, the FPR is $\sim$6\%. This variable contributes negligibly to the inter-study $p$-value divergence.

\paragraph{Statistical framework.}
Holding sample ($N=19$) and survey model (Table~\ref{tab:surveys}) fixed, the $p$-value varies by a factor of $\sim$30 (including the Model~C soft upper bound; $\sim$10 across the four numerically stable methods). Statistical framework choice is the dominant source of inter-study divergence. Sample composition contributes marginally (Table~\ref{tab:pvalues}) and survey selection bias contributes negligibly (FPR$\approx$6\%).
...
Three steps would help. First, every LSST-era ETNO clustering claim should report $p$-values under at least three frameworks: an uncorrected test, a survey-simulation method, and a bootstrap or permutation-based empirical null. The spread is the measurement. Second, analysis plans should be registered before data releases, specifying frameworks, selection criteria, and quality thresholds in advance. This removes the incentive to shop for the most publishable $p$-value after seeing the data. Third, the LSST solar system pipeline should treat cross-survey calibration as a first-order deliverable. The survey's rolling cadence and dynamic footprint mean the selection function changes from night to night; the community needs an open, version-controlled simulator to model it. The pipeline released with this paper provides a baseline. Without a shared protocol, a larger catalog will not settle the debate. It will produce the next generation of incommensurable $p$-values.
