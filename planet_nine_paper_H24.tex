...
Under our public survey model, the false-positive rate for the Rayleigh test at $N=19$ is $5.8\%$ (5,000 injection--recovery trials, seed 20260713; bootstrap sampling noise $\sim$0.3 percentage points). A dedicated parameter-perturbation scan varying survey depth ($\pm0.5$~mag, modelled as base-efficiency shifts of $\pm0.05$) and ecliptic-latitude coverage ($\pm10\%$) shifts the FPR by at most 0.6 percentage points in either direction---well within the paper's conservative claim of $\pm2$ pp (\texttt{fpr\_perturbation\_scan.py}). This FPR estimate includes the combined effects of survey footprint geometry, ecliptic-latitude coverage, and per-object inclination-dependent detection efficiency. It does not include weather losses, chip gaps, or per-night pointing variation---none of which are publicly available for the discovery surveys and all of which would, if included, further reduce the effective detection efficiency without creating directional bias in $\varpi$-space.

If the true $\varpi$ distribution were uniform, the combined footprints of Pan-STARRS, DES, Subaru/HSC, and other discovery surveys would produce a spuriously significant Rayleigh $p$-value in only $\sim$6 in 100 synthetic samples---consistent with the nominal test size. \textbf{Survey selection effects alone are therefore insufficient to explain the $\sim$30$\times$ spread in $p$-values observed across statistical frameworks.} This does not imply that selection effects are absent---only that the scale of the inter-framework divergence cannot be attributed to inhomogeneous sky coverage at the current sample size.

This finding reframes the debate. The question has been structured as ``is the clustering real or a selection artefact?'' Our data indicate that survey selection effects, at their currently modelled strength, cannot fully account for the observed signal. The remaining question---``how strong is the statistical evidence?''---depends on which statistical framework one chooses.
...
On \textbf{observational bias}: our FPR $\sim6\%$ calibrates this dimension. Survey selection effects, at the current sample size and as modelled with publicly available parameters, are insufficient to fully account for the observed clustering. The long-standing suspicion that the signal is entirely a selection artefact---the position associated with OSSOS \cite{shankman2017} and supported by the DES-only analysis \cite{bernardinelli2020}---receives only partial support from the quantitative FPR calibration; the modelled selection effects are real but their magnitude does not match the scale of the inter-framework $p$-value divergence.
...
The ETNO perihelion clustering controversy has persisted for a decade not because the signal is weak or the data insufficient, but because three logically distinct questions have been treated as one. The signal is unlikely to be a pure selection artefact: the survey-induced false-positive rate is $\sim6\%$, consistent with the nominal $\alpha=0.05$ test size, and modelled selection effects are insufficient to explain the $\sim$30$\times$ inter-framework $p$-value spread. The strength of the evidence depends on the statistical framework---five methods applied to the identical 19-object sample yield $p$-values spanning a factor of $\sim$30 (reducing to $\sim$10 across the four numerically stable methods), and three methods confirm clustering at $\alpha=0.05$ (surviving Bonferroni correction for multiplicity), with the sole numerically stable dissenter depending on proprietary code. All quantitative claims are traceable to a single open-source script (\texttt{etno\_simulator.py}); $p$-value confidence intervals and a full parameter-perturbation analysis are provided in the reproducibility package. The physical mechanism---Planet Nine, scattered-disk self-gravity, or primordial alignment---cannot be adjudicated by the current sample and must await LSST.
