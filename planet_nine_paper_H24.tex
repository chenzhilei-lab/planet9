...
Clustering is quantified with the Rayleigh test. The mean resultant length is
\begin{equation}
...
Kuhn called this situation \textit{incommensurability}: competing paradigms that define their terms and standards of evidence so differently that no neutral vantage point exists from which to choose between them \cite{kuhn1962}. The ETNO debate is a near-perfect example. A researcher who accepts Model~A's definition will conclude the evidence is decisive ($p=0.010$). A researcher who insists on Model~B's definition will find it marginal ($p=0.069$). Neither has made a calculation error. They are measuring the same data with different rulers, and the rulers encode the answer.
